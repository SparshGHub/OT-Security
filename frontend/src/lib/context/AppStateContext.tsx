...
export const AppStateProvider = ({ children }: { children: ReactNode }) => {
  ...
  const removeComponent = async (componentId: string) => {
    ...
    // Call API after state update to minimize race conditions
    try {
      await fetchFromApi(`/plant/components/${componentId}`, token, { method: 'DELETE' });
    } catch (err) {
      console.error('Failed to delete component on backend:', err);
    }
  };
  ...
  const removeConnectivity = async (edgeId: string) => {
    ...
    try {
      ...
      console.error('Failed to delete connectivity on backend:', err);
    }
  };
